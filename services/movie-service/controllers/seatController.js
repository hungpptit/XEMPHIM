...
export const createSeats = async (req, res) => {
  try {
    ...
    const response = await axios.post(`${SEAT_SERVICE}/api/seats/bulk`, seats.map(s => ({ ...s, hall_id: hallId })));
    res.status(201).json({
      success: true,
...
export const getSeatsByHall = async (req, res) => {
  try {
    ...
    const response = await axios.get(`${SEAT_SERVICE}/api/seats/hall/${hallId}`);
...
export const getSeatLayout = async (req, res) => {
  try {
    const { hallId } = req.params;
    const response = await axios.get(`${SEAT_SERVICE}/api/seats/hall/${hallId}/layout`);
    res.json(response.data);
  } catch (error) {
...
export const updateSeat = async (req, res) => {
  try {
    ...
    const response = await axios.put(`${SEAT_SERVICE}/api/seats/${seatId}`, req.body);
    res.json({
      success: true,
...
export const updateSeatType = async (req, res) => {
  try {
    const { hallId } = req.params;
    const response = await axios.put(`${SEAT_SERVICE}/api/seats/hall/${hallId}/type`, req.body);
    res.json(response.data);
  } catch (error) {
...
export const deleteSeat = async (req, res) => {
  try {
    const { seatId } = req.params;
    const response = await axios.delete(`${SEAT_SERVICE}/api/seats/${seatId}`);
    res.json({
      success: true,
